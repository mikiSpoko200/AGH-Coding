...
def f_logic(x: float, b1: bool, b2: bool):
...
def f_comprehensions(dct: Dict[int, int]) -> Tuple[int, Set[int]]:
    return reduce(lambda acc, elem: acc + elem, [num for num in range(2, 11) if num != 5 and num != 7]), \
           {key for key in dct.keys() if 1 <= key <= 5}
